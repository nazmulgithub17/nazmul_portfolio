# 🌐 Nazmul Hassan – Personal Portfolio Website

A modern, animated, and fully responsive personal portfolio website built using **HTML, CSS, and Vanilla JavaScript**.  
This portfolio showcases my **education, skills, and contact information** with a visually rich UI and smooth animations.

---

## 👋 About Me

Hi! I'm **Nazmul Hassan**,  
an **Educational Technology & Engineering student** from Bangladesh and an aspiring developer.  
I love learning new technologies, building creative web interfaces, and developing meaningful digital solutions.

---

## 🚀 Features

- ✨ Modern & attractive UI/UX  
- 📱 Fully responsive (Mobile, Tablet, Desktop)  
- 🎨 Animated background (Stars & Floating Orbs)  
- 🖱️ Custom cursor effect  
- 📜 Smooth scrolling navigation  
- 🎓 Education timeline  
- ⚡ Skills cards with hover animation  
- 🔝 Scroll-to-top button  
- 📬 Social media & contact integration  

---

## 🛠️ Technologies Used

- **HTML5**
- **CSS3**
- **JavaScript (Vanilla)**
- **CSS Grid & Flexbox**
- **Intersection Observer API**

---

## 📂 Project Structure


---

## 🔗 Sections Included

- 🏠 Home  
- 🙋 About Me  
- 🎓 Education  
- ⚡ Skills  
- 📬 Contact  

---

## 🌐 Live Demo

Check out the live portfolio here:  
➡️ [Nazmul Hassan Portfolio](https://nazmul-portfolio-nine.vercel.app)  

---

## 📞 Contact Information

- 📱 Phone / WhatsApp: **+880 1754780796**
- 🌐 Facebook: [md.nazmul.hassan0](https://www.facebook.com/md.nazmul.hassan0)
- 📸 Instagram: [md.nazmul.hassan0](https://www.instagram.com/md.nazmul.hassan0)
- 🐦 Twitter (X): [MdNazmulHassan0](https://x.com/MdNazmulHassan0)

---

## 🧠 What I Learned

- Advanced CSS animations & gradients  
- Responsive web design  
- JavaScript DOM manipulation  
- Scroll-based animation techniques  
- Clean UI structure for portfolios  

---

## 🚧 Future Improvements

- Add Projects section  
- Add Contact form  
- Add Dark / Light mode  
- Improve SEO & performance  

---

## 📄 License

This project is open-source and free to use for learning and personal portfolios.  
If you use this project, please give proper credit.

---

## 💙 Author

**Nazmul Hassan**  
Designed & Developed with passion 💙  
© 2025 All Rights Reserved
